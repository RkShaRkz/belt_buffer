...
local LOADER_ADJUSTMENT = 0.25
local SPLITTER_AREA_ADJUSTMENT = 2

---- debug method
local function stringify(var)
	local retval = ''
	if (var == nil) then
		retval = retval .. "null"
	elseif (type(var) == "table") then
		retval = retval .. serpent.block(var)
	else
		retval = retval .. tostring(var)
	end
	return retval
end
-----
...
--		game.print("Entity built: "..stringify(entity.name))
...
--			game.print(direction)
		...
		if direction == 0 then
			loader_in = surface.create_entity{
				...
				position = {x = position.x, y = position.y + LOADER_ADJUSTMENT},
				direction = 0,
				force = force
			}
		...
		elseif direction == 2 then
			loader_in = surface.create_entity{
				name = "belt-buffing-loader",
				position = {x = position.x - LOADER_ADJUSTMENT, y = position.y},
				direction = 2,
				force = force
			}
			loader_out = surface.create_entity{
...
--		game.print("loader in: "..stringify(loader_in).." is valid? "..stringify(loader_in.valid))
--		game.print("loader out: "..stringify(loader_out).." is valid? "..stringify(loader_out.valid))
...
--			game.print("Something went wrong, check debug logs to figure out what")
			if loader_out then
				loader_out.destroy()
			end
			if loader_in then
...
remove_nearby_loaders = function(name, position, surface)
	if name == "belt-buffer-vu" or name == "belt-buffer-vd" then
		local p1 = {x = position.x, y = position.y - LOADER_ADJUSTMENT}
		local splitter_area = {{p1.x - SPLITTER_AREA_ADJUSTMENT, p1.y - SPLITTER_AREA_ADJUSTMENT},{p1.x + SPLITTER_AREA_ADJUSTMENT, p1.y + SPLITTER_AREA_ADJUSTMENT}}
...
--				game.print("killed him")
		...
		local p2 = {x = position.x, y = position.y + LOADER_ADJUSTMENT}
		splitter_area = {{p2.x - SPLITTER_AREA_ADJUSTMENT, p2.y - SPLITTER_AREA_ADJUSTMENT},{p2.x + SPLITTER_AREA_ADJUSTMENT, p2.y + SPLITTER_AREA_ADJUSTMENT}}
		local splitters = surface.find_entities_filtered{area = splitter_area, type = "splitter"}
...
--				game.print("killed his friend")
	...
	if name == "belt-buffer-hl" or name == "belt-buffer-hr" then
		local p1 = {x = position.x - LOADER_ADJUSTMENT, y = position.y}
		local area = {{p1.x - 0.01, p1.y-0.01},{p1.x+0.01, p1.y+0.01}}
		local splitter_area = {{p1.x - SPLITTER_AREA_ADJUSTMENT, p1.y - SPLITTER_AREA_ADJUSTMENT},{p1.x + SPLITTER_AREA_ADJUSTMENT, p1.y + SPLITTER_AREA_ADJUSTMENT}}
		local splitters = surface.find_entities_filtered{area = splitter_area, type = "splitter"}
		for k, splitter in pairs (splitters) do
...
--				game.print("killed him")
		...
		local p2 = {x = position.x + LOADER_ADJUSTMENT, y = position.y}
		splitter_area = {{p2.x - SPLITTER_AREA_ADJUSTMENT, p2.y - SPLITTER_AREA_ADJUSTMENT},{p2.x + SPLITTER_AREA_ADJUSTMENT, p2.y + SPLITTER_AREA_ADJUSTMENT}}
		local splitters = surface.find_entities_filtered{area = splitter_area, type = "splitter"}
		local area = {{p2.x - 0.01, p2.y-0.01},{p2.x+0.01, p2.y+0.01}}
		local nearby = surface.find_entities_filtered{area = area, name = "belt-buffing-loader"}
...
----------------------------------------------------------------------------------
-- will probably get rid of this, because when other people break this mod they --
-- should fix it to remain compatible instead od authors/maintainers doing it	--
----------------------------------------------------------------------------------
...
lib.events =
{
	...
	[defines.events.on_player_mined_entity] = mined_entity,
